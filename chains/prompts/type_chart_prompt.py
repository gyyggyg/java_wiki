...
3. 综合层级位置、职责范围及与其他模块的关联，为 `module_information` 中出现的每个 nodeId 生成一个中文名称，要求：
   - 不要使用驼峰命名法
   - 使用自然语言、可读性强、空格分隔、一眼看懂
   - 高度展示概括模块的具体功能，体现差异和联系
   - 体现上下级关系
   - 同级名称保持相关性
   - 与依赖模块名称呼应
   - 避免模棱两可功能不清晰的名称，如 "模块1"、"基础模块A"
   - 尽量以"模块"作为名称的结尾
...
MODULE_NAME_SHORTEN_PROMPT = PromptTemplate(
    input_variables=["current_tree", "other_names"],
    template="""
你是模块命名精简专家。下面是一棵模块树的草稿名称，请精简。

【输入1：当前树】（需要精简的树，含 nodeId、草稿名 name、子模块 children，部分节点含 explanation 语义说明）
{current_tree}

【输入2：其他树已有的名字】（仅供避免重名参考，不需要精简）
{other_names}

【核心原则】子模块的名字不需要重复父模块已经表达的上下文，读者是在树中浏览，能看到父节点。

【精简方法】
1. 砍掉空泛修饰词：统一、全流程、聚合、综合、核心、一体化、集中、整合、多维度、全链路
2. 同级兄弟用最短的区分词。例如同级接口：权限、订单、商品、优惠券
3. 不以"模块"结尾，除非去掉后语句不通顺
4. 所有名称不得与"其他树已有的名字"重复，全局唯一
5. 相似节点必须基于 explanation 语义区分，禁止用编号（如"辅助一""辅助二"）
6. 父子节点名字不能仅差一两个字，子节点应体现自己独特的职责

【示例】精简前 → 精简后：
- "商城后台管理系统核心模块" → "后台管理系统"
  - "商城后台业务统一接口模块" → "后台业务接口"
    - "权限与角色统一管理接口模块" → "权限与角色接口"
    - "优惠券及历史记录管理接口模块" → "优惠券接口"
    - "订单与售后统一管理接口模块" → "订单与售后接口"
  - "商城后台业务数据访问模块" → "后台数据访问"
  - "业务数据传输对象规范模块" → "业务传输规范"
    - "管理员账号安全参数对象模块" → "管理员账号安全模块"
    - "订单操作参数与结果对象模块" → "订单操作与结果模块"

【输出】只输出当前树中所有节点的精简名（严格JSON，不要包含任何其他解释、前言或Markdown标记）
{{"nodeId1":"精简名","nodeId2":"精简名",...}}
"""
)
