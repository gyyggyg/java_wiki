...
def _ensure_mcp_config(mcp_config_path: str) -> str:
    """确保 mcp_config.json 存在。不存在则根据当前 Python 解释器 + interfaces/neo4j_mcp_interface.py
    自动生成一份，使 Claude CLI 能加载 neo4j MCP。"""
    if os.path.isfile(mcp_config_path):
        return mcp_config_path

    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    neo4j_script = os.path.join(project_root, "interfaces", "neo4j_mcp_interface.py")
    if not os.path.isfile(neo4j_script):
        logger.warning(f"[BizAgent] 未找到 neo4j MCP 脚本: {neo4j_script}，跳过自动生成配置")
        return mcp_config_path
    ...
    config = {
        "mcpServers": {
            "neo4j": {
                ...
                "args": [neo4j_script],
                "env": {
                    "WIKI_NEO4J_URI": os.environ.get("WIKI_NEO4J_URI", ""),
                    "WIKI_NEO4J_USER": os.environ.get("WIKI_NEO4J_USER", "neo4j"),
                    "WIKI_NEO4J_PASSWORD": os.environ.get("WIKI_NEO4J_PASSWORD", ""),
                },
            }
        }
    }
    try:
        os.makedirs(os.path.dirname(mcp_config_path) or ".", exist_ok=True)
        with open(mcp_config_path, "w", encoding="utf-8") as f:
            json.dump(config, f, ensure_ascii=False, indent=2)
        logger.info(f"[BizAgent] 已自动生成 MCP 配置: {mcp_config_path} "
                    f"(python={sys.executable}, script={neo4j_script})")
    except Exception as e:
        logger.warning(f"[BizAgent] 自动生成 MCP 配置失败: {e}")
    return mcp_config_path
