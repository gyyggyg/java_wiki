...
§1 业务概述 / §2 数据结构与架构概览 / §3 对外接口清单 / §4 关键协作时序图
§5 核心业务流程图 / §6 业务状态流转 / §7 组件索引

LLM 调用统一走 business_flow/llm_client.py 的 invoke_llm_strict / invoke_llm_markdown，
由 generate.py 启动时通过 set_default_llm() 注入。
"""
...
from business_flow.neo4j_fetch import (
    fetch_class_briefs,
    fetch_classes_with_fields,
    search_classes_by_names,
)
from business_flow.llm_client import invoke_llm_strict, invoke_llm_markdown
from business_flow.mermaid_validator import invoke_llm_with_mermaid_check
from business_flow.prompts import (
    ...
    BUSINESS_DICTIONARY_SYSTEM, build_business_dictionary_user_prompt,
)
from business_flow.branch_analyzer import (
    analyze_entry_methods, format_branch_hint,
    analyze_serviceimpl_methods, format_serviceimpl_branch_hint,
)
from business_flow.state_field_detector import (
    detect_state_field_candidates, format_candidates_for_prompt,
)
from business_flow.class_context import build_class_context_text
...
logger = logging.getLogger("business_flow.sections")
...
async def build_overview_section(flow: Dict, scope: Dict, neo4j) -> SectionNode:
    ...
    md, _ = await invoke_llm_markdown(
...
async def build_sequence_section(flow: Dict, scope: Dict,
    ...
    result, sid = await invoke_llm_with_mermaid_check(
...
def _format_missing_data_markdown(flow_name: str, missing_classes: List[Dict], notes: str) -> str:
    """将 LLM 返回的 missing_data 组装为告警 markdown"""
    lines: List[str] = []
    if notes:
        lines.append(notes)
        lines.append("")
...
async def build_data_model_section(flow: Dict, scope: Dict, neo4j) -> SectionNode:
    ...
    if external_names:
        ...
        if found_ids:
            # 只为补查到的新类查字段（不重新查原 scope；LLM 已通过 resume session 见过）
            ...
            result2, sid2 = await invoke_llm_with_mermaid_check(
                system_prompt=DATA_MODEL_SYSTEM,
                user_prompt=user2,
                required_keys=["status"],
...
def _format_skipped_controller_block(items: List[Dict]) -> str:
    """直通 Controller 接口：每项多行（标题 + 用途 + 实现 + 可选字段）。"""
    if not items:
        return ""
    lines: List[str] = [
        "#### 直通查询接口",
        ...
        "下面从**业务用途、实现要点、输入/返回、核心依赖、权限控制**几个维度逐一说明。",
        "",
    ]
    for s in items:
        entry = s.get("entry", "?")
        ...
        inputs = (s.get("inputs") or "").strip()
        returns = (s.get("returns") or "").strip()
        access = (s.get("access_control") or "").strip()
        edge = (s.get("edge_cases") or "").strip()
        deps = s.get("key_dependencies") or []
        lines.append(f"##### `{entry}`")
        lines.append("")
        if purpose:
            lines.append(f"- **业务用途**：{purpose}")
        if impl:
            lines.append(f"- **实现要点**：{impl}")
        if inputs:
            lines.append(f"- **输入**：{inputs}")
        if returns:
            lines.append(f"- **返回**：{returns}")
        if deps:
            dep_str = "、".join(f"`{d}`" for d in deps if d)
            lines.append(f"- **核心依赖**：{dep_str}")
        if access:
            lines.append(f"- **权限控制**：{access}")
        if edge:
            lines.append(f"- **注意事项**：{edge}")
        # 都没填时用 reason/description 兜底（向下兼容老 JSON）
        if not any([purpose, impl, inputs, returns, deps, access, edge]):
...
def _format_skipped_service_block(items: List[Dict],
    ...
    """简单 Service：每项标注调用者（含 Controller 在 §5.X 的编号）+ 详细实现说明。"""
    if not items:
        return ""
    lines: List[str] = [
        "#### 简单 Service 实现",
        ...
        "本身实现薄（单行转发或少量字段赋值），不单独画 CFG，这里完整说明其用途与实现。",
        "",
    ]
    for s in items:
        entry = s.get("entry", "?")
...
async def _generate_single_control_flow(
    ...
    try:
        src_parsed, _ = await invoke_llm_strict(
    ...
    try:
        cfg_parsed, sid_B = await invoke_llm_with_mermaid_check(
    ...
    # ==================== Stage C: 校验行号 ====================
    try:
        val_parsed, _ = await invoke_llm_strict(
...
async def build_control_flow_section(flow: Dict, scope: Dict,
    ...
    Stage 1 规划  —— LLM 读 entry_methods + 类骨架，识别 N 个值得画的控制流及各自需要的类
    ...
    # -------- Stage 0b：ServiceImpl 层方法分支扫描（为 service_flows 识别提供客观依据）--------
    service_branch_analysis = analyze_serviceimpl_methods(all_class_rows, min_branches=2)
    logger.info(
        f"[s5] {flow['name']} ServiceImpl 方法分支扫描: "
        f"{len(service_branch_analysis)} 个含业务逻辑方法"
    )
    ...
    plan_user = build_control_flow_plan_user_prompt(
        ...
        service_branch_hint=format_serviceimpl_branch_hint(service_branch_analysis),
    )
    try:
        plan, _ = await invoke_llm_strict(
    ...
    if not planned:
        if skipped_controllers or skipped_services:
            # 纯直通场景：没有 CFG 要画，直接在章节下展开两个小节，不再套 `### 5.1` 一级
            parts: List[str] = [
            ...
            skip_nid = {"5": skip_merged} if skip_merged else {}
        else:
...
async def build_components_section(flow: Dict, scope: Dict, neo4j) -> SectionNode:
    ...
    if not core and not support:
        ...
        return SectionNode(title="## 8. 组件索引", content=[text], neo4j_id={})
    ...
    for layer in layer_order:
        ...
        if layer_ids:
            neo4j_id_map[f"8.{layer}"] = layer_ids
    ...
    if support:
        ...
        sub_content.append(TextNode(content={"markdown": "\n".join(sup_lines)}))
        if sup_ids:
            neo4j_id_map["8.support"] = sup_ids
...
async def build_state_machine_section(flow: Dict, scope: Dict,
    ...
    2. 1 次 LLM 调用：让模型从候选里挑 1-3 个真状态机并画 stateDiagram-v2
    ...
    try:
        result, _ = await invoke_llm_with_mermaid_check(
    ...
    logger.info(f"[s6] {flow['name']} LLM 挑出 {len(state_machines)} 张状态机图")

    if not state_machines:
        logger.info(f"[s6] {flow['name']} LLM 判定候选均非真实状态机，跳过本章")
        return None
...
# ===========================================================
# §8 业务字典（Auto-generated via LLM + Neo4j @TableName entity 源码）
# ===========================================================

async def _fetch_table_entity_sources(neo4j, class_ids: List[str]) -> List[Dict]:
    """查 scope 里是 @TableName 的 Class，返回 source_code。
    业务字典只对真实的 DB 实体类做分析，过滤掉 DTO/VO/Request 等视图类。"""
    if not class_ids:
        return []
    ...
    if not ids_int:
        return []
    query = """
    MATCH (c:Class)-[:ANNOTATED_BY]->(:Annotation {name: 'TableName'})
...
def _format_entity_sources_for_dict(rows: List[Dict], max_chars_per_entity: int = 3000) -> str:
    """把实体源码拼成给 LLM 的上下文；超长的实体源码截断"""
    parts = []
    for r in rows:
        name = r.get("name") or "?"
        src = (r.get("source_code") or "").strip()
        if not src:
            continue
        if len(src) > max_chars_per_entity:
            src = src[:max_chars_per_entity] + "\n// ...（源码过长，已截断）"
        file_tag = r.get("file_name") or ""
        parts.append(f"// ==== {name}  [{file_tag}] ====\n{src}")
    return "\n\n".join(parts)


def _render_enum_dicts_md(dicts: List[Dict]) -> str:
    lines: List[str] = []
    for d in dicts:
        title = (d.get("title") or d.get("field") or "未命名字段").strip()
        lines.append(f"\n#### {title}")
        lines.append("")
        lines.append("| 值 | 类别 | 含义 / 履约 | 示例 |")
        lines.append("|---|---|---|---|")
        for v in d.get("values") or []:
            val = v.get("value", "")
            category = v.get("category") or v.get("label") or "-"
            meaning = v.get("fulfillment") or v.get("meaning") or "-"
            example = v.get("example") or "-"
            lines.append(f"| `{val}` | {category} | {meaning} | {example} |")
    return "\n".join(lines)


def _render_field_groups_md(groups: List[Dict]) -> str:
    lines: List[str] = []
    for g in groups:
        title = (g.get("title") or "字段组").strip()
...
def _render_rules_md(rules: List[Dict]) -> str:
    lines: List[str] = []
    for r in rules:
...
async def build_business_dictionary_section(
    flow: Dict, scope: Dict, neo4j,
) -> Optional[SectionNode]:
    """§7 业务字典（枚举字段 / 字段语义组 / 业务规则）

    数据源：scope 里的 @TableName 实体类源码（带字段声明 + Swagger/Javadoc 注释）
    LLM 从源码推断 3 子类型；无任何产出则返回 None（章节被跳过）。
    """
    name = flow["name"]
    logger.info(f"[s-dict] 生成业务字典: {name}")

    class_ids = (scope.get("class_ids") or []) + (scope.get("record_ids") or [])
    rows = await _fetch_table_entity_sources(neo4j, class_ids)
    if not rows:
        logger.info(f"[s-dict] {name} 无 @TableName 实体，跳过业务字典章节")
        return None

    entity_sources = _format_entity_sources_for_dict(rows)
    if not entity_sources:
        return None

    logger.info(f"[s-dict] {name} 喂给 LLM: {len(rows)} 个 @TableName 实体，"
                f"{len(entity_sources)} chars")

    try:
        result, _ = await invoke_llm_strict(
            system_prompt=BUSINESS_DICTIONARY_SYSTEM,
            user_prompt=build_business_dictionary_user_prompt(
                flow_name=name,
                flow_kind=flow.get("kind", "?"),
                entity_sources=entity_sources,
            ),
            required_keys=["enum_dictionaries", "field_semantic_groups", "business_rules"],
            label=f"s-dict-{name}",
        )
    except Exception as e:
        logger.warning(f"[s-dict] {name} LLM 失败: {type(e).__name__}: {e}，跳过章节")
        return None

    enum_dicts = result.get("enum_dictionaries") or []
    field_groups = result.get("field_semantic_groups") or []
    rules = result.get("business_rules") or []

    if not enum_dicts and not field_groups and not rules:
        logger.info(f"[s-dict] {name} LLM 未识别出任何字典内容，跳过")
        return None

    logger.info(
        f"[s-dict] {name} 产出: {len(enum_dicts)} 枚举字段 / "
        f"{len(field_groups)} 字段组 / {len(rules)} 业务规则"
    )
    ...
    sub_counts = []
    if enum_dicts:
        sub_counts.append(f"**{len(enum_dicts)}** 个枚举字段")
    if field_groups:
        sub_counts.append(f"**{len(field_groups)}** 组协作字段")
    if rules:
        sub_counts.append(f"**{len(rules)}** 条业务规则")
    intro_md = (
        f"以下内容由 LLM 从本业务流的 **{len(rows)}** 个 `@TableName` 实体类源码中"
        f"自动提炼得到，共识别 {'、'.join(sub_counts)}。"
        "读代码前先扫一眼这里，可以避免被魔法值和字段命名坑。"
    )
    content.append(TextNode(content={"markdown": intro_md}))

    if enum_dicts:
        content.append(TextNode(content={"markdown": "### 枚举字段字典"}))
        content.append(TextNode(content={"markdown": _render_enum_dicts_md(enum_dicts)}))

    if field_groups:
        content.append(TextNode(content={"markdown": "### 字段语义组"}))
        content.append(TextNode(content={"markdown": _render_field_groups_md(field_groups)}))

    if rules:
        content.append(TextNode(content={"markdown": "### 业务规则（跨字段不变式）"}))
        content.append(TextNode(content={"markdown": _render_rules_md(rules)}))

    # neo4j_id：本章节涉及的实体 class_id（让前端能跳转）
    entity_ids = [str(r.get("class_id")) for r in rows if r.get("class_id") is not None]
    neo4j_id_map = {"7": entity_ids} if entity_ids else {}
