"""Mermaid 语法校验 + 语法错误自动重试（API 版）

调用 `scripts/validate_mermaid.mjs` 子进程（node + jsdom + mermaid.parse），
在 LLM 返回含 mermaid 的结果后做一轮纯语法校验；若失败则把错误反馈
追加到 user prompt 上再调一次 LLM，最多重试 `max_retries` 次。

依赖 `business_flow/llm_client.py` 的 `invoke_llm_strict` 做实际的 LLM 调用 + 重试。
"""
...
from business_flow.llm_client import invoke_llm_strict, Session

logger = logging.getLogger("business_flow.mermaid")
...
def _build_short_fix_prompt(err_msg: str) -> str:
    """resume 语境下的修复提示（极简，LLM 已看到上次输出和原请求）"""
    return (
...
async def invoke_llm_with_mermaid_check(
    system_prompt: str,
    user_prompt: str,
    ...
    for attempt in range(max_retries + 1):
        ...
        last_result, last_session = result, sess
        ...
        if ok:
            if attempt > 0:
                logger.info(f"[{label}] mermaid 语法校验在第 {attempt + 1} 轮通过 (resume={bool(current_session)})")
            return result, sess
    ...
    return last_result, last_session
