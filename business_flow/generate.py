"""
业务流 wiki 生成器 —— LLM API 版本

读取  business_flow/business_flows_with_span.json
输出  business_flow/output/<flow名>.meta.json × N（flow 名末尾"流"字会被省略）

走标准 LangChain + LLMInterface API 调用（OpenAI / Claude / Google 均可，
按 .env 里的 LLM_PROVIDER / LLM_MODEL / BASE_URL / OPENAI_API_KEY 配置）。

运行:
    python business_flow/generate.py
    python business_flow/generate.py --flow "支付宝支付"
    python business_flow/generate.py --concurrency 2
    python business_flow/generate.py --start 5 --end 10   # 只跑输入里第 5~10 个
    python business_flow/generate.py --start 5            # 第 5 个到末尾
    python business_flow/generate.py --end 3              # 前 3 个
"""
...
from business_flow.llm_client import set_default_llm
from business_flow.schema import SectionNode, TextNode
from business_flow.neo4j_fetch import fetch_classes_full_context
from business_flow.class_context import build_class_context_text
from business_flow.sections import (
    ...
    build_business_dictionary_section,
)
from business_flow.assembler import assemble_wiki
...
async def generate_wiki_for_flow(flow: dict, neo4j, logger: logging.Logger) -> dict:
    ...
    #   §6 业务状态流转      (state_machine) —— stateDiagram-v2（可能返回 None 被跳过）
    #   §7 业务字典          (business_dictionary) —— LLM 从 @TableName 实体抽枚举/字段组/规则（可能返回 None）
    #   §8 组件索引          (components) —— 纯模板；永远作为最后一章
    results = await asyncio.gather(
        build_overview_section(flow, scope, neo4j),                                     # s1 → §1
        build_data_model_section(flow, scope, neo4j),                                  # s2 → §2
        build_entrypoints_section(flow, scope, class_context_text, all_class_rows),     # s3 → §3
        build_sequence_section(flow, scope, class_context_text, all_class_rows, neo4j), # s4 → §4
        build_control_flow_section(flow, scope, all_class_rows, neo4j),                 # s5 → §5
        build_state_machine_section(flow, scope, all_class_rows, neo4j),                # s6 → §6
        build_business_dictionary_section(flow, scope, neo4j),                           # s7 → §7
        build_components_section(flow, scope, neo4j),                                   # s8 → §8
        return_exceptions=True,
    )
    section_nodes = []
    s6_skipped = False
    s7_skipped = False
    for sname, r in zip(["s1", "s2", "s3", "s4", "s5", "s6", "s7", "s8"], results):
        if isinstance(r, Exception):
            ...
            # §6 state_machine / §7 business_dictionary 允许返回 None
    ...
    # —— 章节编号重整（保持连续，并且让组件索引始终是最后一章）——
    # 原编号: §6 state_machine / §7 business_dict / §8 components
    # 按跳过情况移位：
    #   s6 ok,  s7 ok  → 不动（§1-§8）
    #   s6 skip, s7 ok → §7→§6, §8→§7
    #   s6 ok,  s7 skip → §8→§7
    #   s6 skip, s7 skip → §8→§6
    renumber_map: Dict[int, int] = {}
    if s6_skipped and not s7_skipped:
        renumber_map[7] = 6
        renumber_map[8] = 7
    elif s6_skipped and s7_skipped:
        renumber_map[8] = 6
    elif not s6_skipped and s7_skipped:
        renumber_map[8] = 7

    for sec in section_nodes:
        if not isinstance(sec, SectionNode):
            continue
        for old, new in renumber_map.items():
            prefix_old = f"## {old}."
            if sec.title.startswith(prefix_old):
                sec.title = f"## {new}." + sec.title[len(prefix_old):]
                # 同步 neo4j_id 的键：首段数字的 old→new（形如 "7" 或 "7.xxx"）
                new_nid: Dict[str, Any] = {}
                old_str = str(old)
                new_str = str(new)
                ...
                logger.info(f"[{name}] 章节重编号: §{old} → §{new}（因 s6/s7 跳过）")
                break
...
async def main():
    parser = argparse.ArgumentParser(description="用 LLM API 为每个业务流生成 wiki")
    ...
    parser.add_argument("--concurrency", type=int, default=1,
                        help="并发 flow 数（考虑 LLM 速率限制，建议 1-5）")
    parser.add_argument("--input", default=INPUT_PATH,
                        help="business_flows_with_span.json 路径（默认本模块下）")
    parser.add_argument("--output-dir", default=OUTPUT_DIR,
                        help="输出目录（默认 business_flow/output/）")
    ...
    logger.info(f"input={input_path}, output_dir={output_dir}")

    # 初始化 LLM（按 .env 里的 LLM_PROVIDER / LLM_MODEL / BASE_URL / OPENAI_API_KEY 配置）
    llm = LLMInterface()
    set_default_llm(llm)
    logger.info(
